...
- Cut document management process from 10 minute to 2 minute average with VBA and JavaScript programming; scraping the web-based document system, sorting files, adding data to local database, and interfacing with ProjectWise API to upload file directories to local database.
...
- Created Power BI dashboard from queried scan data to visualize trailer volumes in real-time.
...
- Authored report on tardy package arrival causes, employing data from Oracle database to pinpoint late-arriving accounts and recommend revised pickup schedules for high-volume, local accounts as a possible solution.
...
- Deployed new conveyor hardware with corporate industrial engineering, conveyor manufacturer, and plant engineering.
